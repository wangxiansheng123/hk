...
(function () {
    ...
    function draw(opts) {
        ...
        for (let i = 0; i < num; i++) {
            ...
            // 颜色间隔
            if (i % 2 == 0) {
                ctx.fillStyle = '#fbf5ac';
            } else {
                ctx.fillStyle = '#fae48e';
            }
            ...
            // 奖项列表
            html.push('<li class="gb-turntable-item"><span style="' + transform + ': rotate(' + i * turnNum + 'turn)">' + opts.prizes[i].prizeName + (opts.prizes[i].prizeImgUrl == '' ? '' : '<img src="' + opts.prizes[i].prizeImgUrl + '" />') + '</span></li>');
            if ((i + 1) === num) {
